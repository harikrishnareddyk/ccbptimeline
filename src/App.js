...
import ProjectTimeline from './components/ProjectTimelineCard'
import CourseTimelineCard from './components/TimelineView'
...
const App = () => {
  const renderdata = each => {
    if (each.categoryId === 'PROJECT') {
      return <ProjectTimeline each={each} />
    }
  ...
  return (
    <div className="chrono-container">
      ...
      <Chrono mode="VERTICAL" items={timelineItemsList}>
        {timelineItemsList.map(each => renderdata(each))}
      </Chrono>
    </div>
  )
}
